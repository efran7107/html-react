...
const getTop = () => {
    ...
    .sort((a,b) => a-b)
    .splice(-5, )
    .reverse().map(vote => data.find(char => char.votes === vote))
    .map(char => ({name: char.name, skillset: char.skillset, votes: char.votes})); 
}

const getCardData = () => {
    return data.map(char => ({name: char.name, nickName: char.nickName, imageUrl: char.imageUrl, background: char.background}))
}
